...
<style>
  :root {{
    --bg:#0b1220; --card:#121a2b; --text:#e6e6f0; --muted:#a9b1c7; --border:#223059;
    --row1:#0f1730; --row2:#101a35; --hl:#1c2847;
    --ad:#7c5cff; --sd:#30c2ff; --sid:#f9a835; --ug:#57c957;
  }}
  * {{ box-sizing:border-box; }}
  body {{ background:var(--bg); color:var(--text); font:14px/1.45 system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Arial; margin:0; padding:24px; }}
  .wrap {{ max-width:1220px; margin:0 auto; }}
  h1 {{ font-size:20px; margin:0 0 10px; }}
  .meta {{ color:var(--muted); margin-bottom:14px; }}
  .tools {{ display:flex; gap:12px; align-items:center; margin:12px 0 16px; }}
  input[type="search"] {{
    flex:1; padding:10px 12px; background:var(--card); border:1px solid var(--border); color:var(--text);
  ...
  thead th {{
    position:sticky; top:0; background:linear-gradient(180deg,#162241,#121a2b);
    color:#eaf2ff; text-align:left; padding:12px; font-weight:600; user-select:none; cursor:pointer; border-bottom:1px solid var(--border);
  }}
  ...
  .pill {{ background:#0b4; color:white; padding:2px 8px; border-radius:999px; font-size:12px; margin-left:8px; }}
  .missing {{ color:#ffb8b8; }}
  a {{ color:#a8c9ff; text-decoration:none; }}
  a:hover {{ text-decoration:underline; }}
  .chip {{
    display:inline-block; padding:3px 8px; margin:2px 4px 2px 0; border-radius:999px; text-decoration:none; color:#fff;
    border:1px solid var(--border); font-size:12px; background:#1b2542;
  }}
  .chip-AD {{ border-color:var(--ad); }}
  .chip-SD {{ border-color:var(--sd); }}
  .chip-SID{{ border-color:var(--sid); }}
  .chip-UG {{ border-color:var(--ug); }}
  .coll summary {{ cursor:pointer; list-style:none; }}
  .coll summary::-webkit-details-marker {{ display:none; }}
  .coll summary::before {{ content:'▸'; display:inline-block; margin-right:6px; color:#9fb4ff; }}
  .coll[open] summary::before {{ content:'▾'; }}
  .badge {{
    display:inline-block; min-width:18px; padding:0 6px; text-align:center; border-radius:10px; background:#2a3a68; color:#eaf2ff; font-size:12px;
